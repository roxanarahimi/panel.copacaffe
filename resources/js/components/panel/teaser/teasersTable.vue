...
<!--                    <th class="d-none d-md-table-cell" style="width: 100px" scope="col">تصویر</th>-->
                ...
                <tr :id="'row_'+data.id" v-for="(data, index) in allData" :key="data.id" :data-index="index">
                    <td>{{ index + 1 }}</td>
                    <td>
                        <router-link :to="'/panel/teaser/'+data.id">{{ data.title }}</router-link>
                    </td>
                   ...
                   <td class="d-none d-md-table-cell date_cell">{{ data.created_at }}</td>
                    ...
                    <td>
                        ...
                        <ul class="dropdown-menu" aria-labelledby="navbarScrollingDropdown">
                            ...
                            <!--                                            </li>-->
                            <li>
                                <router-link :to="'/panel/teaser/'+data.id" class="dropdown-item"
                                             style="text-align: right !important">
                                    مشاهده
                                </router-link>
                                <router-link :to="'/panel/edit/teaser/'+data.id" class="dropdown-item"
                                             style="text-align: right !important">
                                    ویرایش
                                </router-link>
                                <a class="dropdown-item" @click="showDeleteModal(data.id)"
                                   style="text-align: right !important" data-bs-toggle="modal"
